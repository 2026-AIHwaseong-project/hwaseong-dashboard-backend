...
def _trips_kpi(p: str, quad_arr, daytype: str = "wd") -> tuple:
    ...
    daytype="we" 면 주말 셀 사전을 읽는다. 주말 quad 배열(weekendImpact·주말
    시뮬레이션)에 평일 셀을 섞으면 wd/we 의 flowTripsPerDay 가 갈라지는 순간
    델타가 어긋난다 — 현재 배포 데이터는 두 값이 같아 티가 안 났을 뿐이다.
    (같은 결론에 두 갈래가 독립 도달해 병합에서 daytype 인자로 통일했다.)
    """
