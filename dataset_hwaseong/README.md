...
### 2-1. 노선이 안 붙는 정류장이 25% 있습니다 🟡 (완화)

`route_stops.csv`(TAGO)는 정류장의 **69.7%** 만 덮습니다. 나머지는 노선 정보가 없어
운행빈도가 0 으로 잡히는데, 실제로는 버스가 옵니다(그 정류장들 승차 합계 120만).
정류장 수로는 25% 지만 **승차량 비중은 5.9%** 입니다.

`routes_village.csv` 로 메우려 했으나 **경유정류소도 좌표도 없어**(노선번호·기점·종점·
배차뿐) 정류장에 붙일 수 없습니다.

→ `03_join.py` 는 **같은 읍면동에서 노선이 붙은 정류장들의 중앙값**으로 대체하고
(718개), `stops_hwaseong.csv` 의 `freq_imputed` 컬럼에 표시합니다.
승하차가 0 인 정류장은 대체하지 않습니다 — 진짜로 버스가 안 오는 곳입니다.
